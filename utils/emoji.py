...
class Emoji(object):
    ...
    EMO = {'GRINNING_FACE_WITH_SMILING_EYES': b'\xF0\x9F\x98\x81',
    ...
    def __init__(self):
        d        = self.EMO
        self.EMO_LIST = [e for _,e in d.iteritems()]
        self.EMO_NAME = dict([(v,'<' + k + '>') for k,v in d.iteritems()])

    '''
        given text, check if it is an emoji
    '''
    def is_emo(self, text):
        return text in self.EMO_LIST

    '''
        given text, remove emoji
    '''
    def remove_emo(self,text):
        xs = text.split(' ')
        ys = []
        for x in xs:
            if self.is_emo(x): pass
            else: ys.append(x)
        return ' '.join(ys)

    '''
        given text, map emoji to a unicode safe token
    '''
    def map_emo(self, text):
        xs = text.split(' ')
        ys = []
        for x in xs:
            if self.is_emo(x): 
                ys.append(self.EMO_NAME[x])
            else: 
                ys.append(x)
        return ' '.join(ys)
